...
{
	PXR_LOGI(PxrUnreal, "UpdateStereoRenderingParams");
	...
	*EyeLayerFound = MakeShareable(EyeLayer);
	...
	PXR_LOGI(PxrUnreal, "vr.MobileMultiView=%d, bIsUsingMobileMultiView=%d", bIsMobileMultiViewEnabled, bIsUsingMobileMultiView);
...

	{
		...
		PXR_LOGI(PxrUnreal, "GetConfig RenderTextureSize:(%d,%d)", RenderTextureX, RenderTextureY);

		if (RenderTextureX == 0 && RenderTextureY == 0)
		{
			RenderTextureX = 4096;
			RenderTextureY = 2048;

			PXR_LOGI(PxrUnreal, "Reset RenderTextureSize:(%d,%d)", RenderTextureX, RenderTextureY);
		}
		...
		{
			PXR_LOGI(PxrUnreal, "The max RenderTexture size is (4160,2080) if the Device is G2/G2 4K !");
		...
		const bool EnableSubsampled = CVarPICOEnableSubsampledLayout.GetValueOnAnyThread() == 1;
...
bool FPICOXRHMD::InitializeSession()
{
	...
	{
		//SetAppEngineInfo2
		FString UnrealSDKVersion = "UE4_2.1.4.3";

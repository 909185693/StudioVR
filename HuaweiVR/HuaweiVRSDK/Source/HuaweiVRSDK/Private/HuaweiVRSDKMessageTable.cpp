...
static const FString m_text_thermalWarningContent_zh_0 = TEXT("手机温度偏高。为保证使用体验，建议将手机放置在散热较好之处");
static const FString m_text_thermalWarningContent_zh_1 = TEXT("手机温度过高。建议将手机放置在散热较好处，或等待手机降温至正常");
static const FString m_text_thermalWarningContent_zh_2 = TEXT("手机温度过高，");
static const FString m_text_thermalWarningContent_zh_3 = TEXT("秒后将退出VR模式。退出后，请等待手机降温至正常之后再体验VR");
static const FString m_text_thermalWarningContent_en_0 = TEXT("Your phone is too hot. Please move it to a well-ventilated place to ensure optimal performance.");
static const FString m_text_thermalWarningContent_en_1 = TEXT("Your phone is too hot. Please move it to a well-ventilated place or wait for it to cool down.");
static const FString m_text_thermalWarningContent_en_2 = TEXT("Your phone is too hot. VR mode will exit in ");
static const FString m_text_thermalWarningContent_en_3 = TEXT(" second. Please wait for the phone to cool down before continuing to use VR.");
...
static const FString m_text_PhoneLowPowerMsg_en_0 = TEXT("Phone battery level is below 10%, please connect the charger.");
static const FString m_text_PhoneLowPowerMsg_zh_1 = TEXT("手机电量不足5%，即将自动关机");
static const FString m_text_PhoneLowPowerMsg_en_1 = TEXT("Phone battery level is below 5%. It will automatically power off in seconds.");
...
static const FString m_text_HelmetLowPowerMsg_en_0 = TEXT("Headset battery level is below 20%, please connect the charger.");
static const FString m_text_HelmetLowPowerMsg_zh_1 = TEXT("头盔电量过低，即将自动关机");
static const FString m_text_HelmetLowPowerMsg_en_1 = TEXT("Headset battery level is too low. It will automatically power off in seconds.");
...
bool HuaweiVRSDKMessageTable::CreateThermalMessage(TMap<FString, FString>& message) {
    ...
    if (0 == HVR_Android_GetThermalWarningInfo(level, frameRate, ratioWidth, ratioHeight)) {
        if (1 == level) {
            ...
            FString content = "";
            if (Language.StartsWith("zh_CN")) {
                message.Add(HuaweiVRMessageKey_Content, content.Append(m_text_thermalWarningContent_zh_2).Append("10").Append(m_text_thermalWarningContent_zh_3));
                message.Add(HuaweiVRMessageKey_Tips, m_text_closeDialogTips_zh);
            } else /*if (Language.StartsWith("en"))*/ {
                message.Add(HuaweiVRMessageKey_Content, content.Append(m_text_thermalWarningContent_en_2).Append("10").Append(m_text_thermalWarningContent_en_3));
                message.Add(HuaweiVRMessageKey_Tips, m_text_closeDialogTips_en);
            } /*else {
                //Todo
            }*/
        ...
        //message.Add(HuaweiVRMessageKey_Extention, FString::Printf__VA(L"%d##%d##%d##%d", level, frameRate, ratioWidth, ratioHeight));
        ret = true;
    }

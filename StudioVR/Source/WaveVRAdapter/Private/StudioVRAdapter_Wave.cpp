...
#include "StudioVRAdapter_Wave.h"
...
void FStudioVRAdapter_Wave::Construction(UObject* Outer, const class FObjectInitializer& ObjectInitializer)
{
#if (PLATFORM_ANDROID || WAVEVR_WINDOWS_TEST)
	WaveVREventCommon = ObjectInitializer.CreateDefaultSubobject<UWaveVREventCommon>(Outer, TEXT("WaveVREventCommon"));

	FCoreDelegates::ApplicationWillEnterBackgroundDelegate.AddRaw(this, &FStudioVRAdapter_Wave::ApplicationWillEnterBackground);
#endif
}

void FStudioVRAdapter_Wave::Register(AStudioVRPawn* NewPawn)
{
...
void FStudioVRAdapter_Wave::Unregister()
{
...
void FStudioVRAdapter_Wave::Tick(float DeltaTime)
{
#if (PLATFORM_ANDROID || WAVEVR_WINDOWS_TEST)
	if (!bWaveVRSetupDone)
	{
...
bool FStudioVRAdapter_Wave::PollControllerState(EControllerHand TrackingSource, FVector& Position, FRotator& Orientation)
{
	// 兼容3Dof控制器，模拟定位
	if (bWaveVRSetupDone)
	{
...
void FStudioVRAdapter_Wave::AdaptationDisplayComponent(UAdaptiveControllerComponent* AdaptiveControllerComponent)
{
...
FVector FStudioVRAdapter_Wave::GetCameraOffset() const
{
	return FVector(0.f, 0.f, 180.f);
}

#if (PLATFORM_ANDROID || WAVEVR_WINDOWS_TEST)
void FStudioVRAdapter_Wave::ApplicationWillEnterBackground()
{
	if (WaveVREventCommon != nullptr)
	{

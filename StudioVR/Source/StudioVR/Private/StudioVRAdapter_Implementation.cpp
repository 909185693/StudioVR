...
void FStudioVRAdapter_Implementation::Register(AStudioVRPawn* NewPawn)
{
	StudioVRPawn = NewPawn;
}

void FStudioVRAdapter_Implementation::Unregister()
{
	StudioVRPawn = nullptr;
}

FVector FStudioVRAdapter_Implementation::GetCameraOffset() const
{
	return FVector::ZeroVector;
}

uint8 FStudioVRAdapter_Implementation::GetDeviceType() const
{
	return EDeviceType::Defualt;
}

void FStudioVRAdapter_Implementation::ClearRenderComponent(class USceneComponent* InParent)
{
	if (InParent != nullptr)
	{
...
UStaticMeshComponent* FStudioVRAdapter_Implementation::CreateRenderComponent(USceneComponent* InParent, FName Name, const TCHAR* Filename, const FTransform& RelativeTransform, const TArray<UMaterialInterface*>& OverrideMaterials)
{

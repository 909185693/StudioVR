...
public:
	...
	* @param	OutPosition		(out) Current position of the device
	* @param	OutOrientation		(out) Current rotation of the device
	...
	*
	* @param	DeviceType		Hmd,Left,Right,Basestation		
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "NOLO VR")
		static void HapticWithArg(EControllerHand Hand, float duration, float frequency, float amplitude);
	
	UFUNCTION(BlueprintCallable, Category = "NOLO VR")
		static void HapticWithAudioFile(FString path, int channel_number);
	
	UFUNCTION(BlueprintCallable, Category = "NOLO VR")
		static void SetHapticAudioConfig(int layout, int format, int sample_rate, int channel_number);
	
	UFUNCTION(BlueprintCallable, Category = "NOLO VR")
		static void HapticWithAudioBuffer(TArray<uint8> buffer, int nb_samples, int channel_number);
	
	UFUNCTION(BlueprintCallable, Category = "NOLO VR")
		static bool StopHaptic(int channel_number);

	UFUNCTION(BlueprintCallable, Category = "NOLO VR")
		static bool M1Write(TArray<uint8> buffer,int32 size);
	
		static bool M1Write(uint8 *buffer,uint32_t size);

	UFUNCTION(BlueprintCallable, Category = "NOLO VR")
		static bool CopyFilesToDevices(FString FilesPath, FString FileName, FString &TargetPath);

...
int FNoloVRHMD::GetNoloProductType()const {
	return noloVrApi.get()->GetNoloProductType();
}

void FNoloVRHMD::HapticWithArg(EControllerHand Hand, float duration, float frequency, float amplitude)
{
	return (Hand == EControllerHand::Left) ? noloVrApi.get()->HapticWithArg( 1,  duration,  frequency,  amplitude): noloVrApi.get()->HapticWithArg(2, duration, frequency, amplitude);
}

void FNoloVRHMD::HapticWithAudioFile(char* path, int channel_number)
{
	noloVrApi.get()->HapticWithAudioFile(path,channel_number);
}

void FNoloVRHMD::SetHapticAudioConfig(int layout, int format, int sample_rate, int channel_number)
{
	noloVrApi.get()->SetHapticAudioConfig(layout,format,sample_rate,channel_number);
}

void FNoloVRHMD::HapticWithAudioBuffer(uint8_t* buffer, int nb_samples, int channel_number)
{
	noloVrApi.get()->HapticWithAudioBuffer(buffer,nb_samples,channel_number);
}

bool FNoloVRHMD::StopHaptic(int channel_number)
{
	return  noloVrApi.get()->StopHaptic(channel_number);
}

bool FNoloVRHMD::M1Write(const void* buffer, uint32_t size)
{
	return  noloVrApi.get()->m1Write(buffer, size);
}
...
{
	return noloVrApi.get()->GetPoseByDeviceType(DeviceType).bDeviceIsConnected;
}
...
bool FNoloVRHMD::GetTrackedDevicePositionAndOrientation(FVector& CurrentPosition, FQuat& CurrentOrientation) {
	...
	bHasValidPose = static_cast<bool>(nPose.bDeviceIsConnected);	
...
public:
	...
	bool LoadNoloVRModule() {
		...
		FString LibraryPath = FPaths::Combine(*BaseDir, TEXT("Source/ThirdParty/NOLOLibrary/x64/Release/NoLo_USBHID.dll"));
